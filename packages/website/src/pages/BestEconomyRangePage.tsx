...
export function BestEconomyRangePage() {
  const [pressureAltFt, setPressureAltFt] = useState(CHART_EXAMPLE_5_27.pressureAltFt);
  const [oatC, setOatC] = useState(CHART_EXAMPLE_5_27.oatC);
  const [power, setPower] = useState<PowerPct>(CHART_EXAMPLE_5_27.power);
  const [reserve, setReserve] = useState<ReservePolicy>(CHART_EXAMPLE_5_27.reserve);
      ...
      <Grid size={{ xs: 12, md: 8 }}>
        <Card>
          <CardContent>
            ...
            <Typography variant="caption" color="text.secondary" component="p" sx={{ mt: 1 }}>
              Chart quirks: the range axis carries two offset 50-nm scales on one lattice — the no-reserve family reads 100 nm lower than the reserve scale at the same grid position. The temperature correction is two-sided (+0.7 nm/°C above standard, −1.1 below). 75% curves end near 10,000 ft. Reserve vs no-reserve deltas are only ~5% self-consistent across chart families — read each family absolutely.
            </Typography>
          </CardContent>
        </Card>
      </Grid>
